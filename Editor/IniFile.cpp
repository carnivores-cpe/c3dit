...
IniFile::IniSection& IniFile::IniSection::operator= (const IniFile::IniSection& rhs)
{
	...
	for (auto s : rhs.m_keys)
	{
		m_keys.insert(m_keys.end(), s);
	}
...
bool IniFile::Save(const std::string& file_name)
{
	...
	fs << "; Auto-generated INI by the IniFile class library." << std::endl;
	fs << std::endl;
	...
	for (auto const& section : m_sections)
	{
		...
		for (auto const& key : section.second.m_keys)
		{
			fs << key.first << " = " << key.second << std::endl;
		}
...
template <> signed IniFile::GetKey(const std::string& section_name, const std::string& key_name, const signed& default_value)
{
	const auto section_i = this->m_sections.find(section_name);
...
template <> float IniFile::GetKey(const std::string& section_name, const std::string& key_name, const float& default_value)
{
	const auto section_i = this->m_sections.find(section_name);
...
template <> std::string IniFile::GetKey(const std::string& section_name, const std::string& key_name, const std::string& default_value)
{
	const auto section_i = this->m_sections.find(section_name);

	if (section_i != this->m_sections.end())
	{
		return section_i->second.GetKey(key_name);
	}
...
template<typename T> void IniFile::SetKey(const std::string& section_name, const std::string& key_name, const T& value)
{
	...
	if (section_i == this->m_sections.end())
	{
		auto r = this->m_sections.insert(std::make_pair(section_name, IniSection(section_name)));
		section_i = r.first;
	}

	std::ostringstream oss;
	oss << value;
	section_i->second.SetKey(key_name, oss.str());
	this->m_updated = true;
}


template <> void IniFile::SetKey(const std::string& section_name, const std::string& key_name, const std::string& value)
{
	this->m_updated = false;
	auto section_i = this->m_sections.find(section_name);
	...
	section_i->second.SetKey(key_name, value);
	this->m_updated = true;
}
...
template <> void IniFile::SetKey(const std::string& section_name, const std::string& key_name, const signed& value)
{
	this->m_updated = false;
	auto section_i = this->m_sections.find(section_name);
...
template <> void IniFile::SetKey(const std::string& section_name, const std::string& key_name, const float& value)
{
	this->m_updated = false;
	auto section_i = this->m_sections.find(section_name);
